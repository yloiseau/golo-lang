...
function test_add = {
  ...
  assertEquals(add(42): andThen(add(1337)): andThen(add(69))(list[]),
               list[42, 1337, 69])
...
function test_operators = {
  assertEquals(inc(41), 42)
  assertEquals(inc(1.2), 2.2)
  assertEquals(dec(43), 42)
  assertThat(dec(2.2), Matchers.closeTo(1.2, 0.0001))
  ...
  let pred = contains(42)
  assertEquals(pred(list[1, 42, 3]), true)
  assertEquals(pred(list[1, 3]), false)
}
...
function test_io = {
  ...
  let p = ^gololang.functions::id
          : andThen(^gololang.functions::inc)
          : andThen(i1)
          : andThen(i2)
          : andThen(^gololang.functions::inc)
