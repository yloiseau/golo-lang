...
function test_unquote = {
  ...
  assertThat(assign: localReference(): name(), `is("__$$_quoted_a"))
...
function test_try_catch_finally = {
  ...
  assertThat(ir: exceptionId(), `is("__$$_quoted_e"))
...
function test_loop = {
  ...
  assertThat(ir: init(): localReference(): name(), `is("__$$_quoted_i"))
  ...
  assertThat(ir: condition(): left(): name(), `is("__$$_quoted_i"))
  assertThat(ir: condition(): right(): value(), `is(10))

  assertThat(ir: post(): localReference(): name(), `is("__$$_quoted_i"))
  assertThat(ir: post(): expression(): type(), `is(OperatorType.PLUS()))
  assertThat(ir: post(): expression(): left(): name(), `is("__$$_quoted_i"))
...
function test_foreachloop = {
  ...
  assertThat(ir: references(): get(0): name(), `is("__$$_quoted_x"))
  assertThat(ir: references(): get(1): name(), `is("__$$_quoted_y"))
  assertThat(ir: iterable(): name(), `is("f"))
  assertThat(ir: whenClause(): type(), `is(OperatorType.LESS()))
  assertThat(ir: whenClause(): left(): name(), `is("__$$_quoted_x"))
  assertThat(ir: whenClause(): right(): name(), `is("__$$_quoted_y"))
...
function test_closure = {
  ...
  assertThat(r: right(): name(), `is("__$$_quoted_c"))
}
...
function test_destruct = {
  ...
  assertThat(list[r: name() foreach r in ir: references()],
      contains("__$$_quoted_a", "__$$_quoted_b"))
...
function test_coll_comprehension = {
  ...
  assertThat(ir: expression(): left(): name(), `is("__$$_quoted_i"))
  assertThat(ir: expression(): right(): name(), `is("__$$_quoted_a"))
...
function test_local_declaration = {
  ...
  assertThat(ir, isA(BinaryOperation.class))
  assertThat(ir: left(): name(), `is("__$$_quoted_a"))
  assertThat(ir: right(): name(), `is("__$$_quoted_b"))
