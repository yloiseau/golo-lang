...
variables and references whose name starts with `$` are not mangled.
...
FunctionInvocation.of("gololang.ir.namedArgument")
  : withArgs(
...
local function quoteMethodInvocation = |node, p| -> quoteLocalDeclaration(
  gololang.ir.invoke("withArgs")
    : withArgs(quoteNodeArray(node: arguments(), p))
    : on(gololang.ir.invoke("nullSafe")
      : withArgs(ConstantStatement.of(node: isNullSafeGuarded()))
      : on(FunctionInvocation.of("gololang.ir.invoke"): withArgs(ConstantStatement.of(node: name())))),
  node: declarations(), p)
...
local function quoteStruct = |node, p| -> gololang.ir.invoke("members")
...
local function quoteWhenClauses = |element, node, p| {
  var ir = gololang.ir.invoke("otherwise")
    : withArgs(quoteNode(node: `otherwise(), p))
    : on(element)
  foreach c, v in node: clauses() {
    ir = gololang.ir.invoke("then"): withArgs(quoteNode(v, p))
          : on(gololang.ir.invoke("when"): withArgs(quoteNode(c, p))
            : on(ir))
  }
  return ir
}
...
local function quoteClosure = |node, p| {
  var ir = FunctionInvocation.of("gololang.ir.lambda")
    : withArgs(node: target(): parameterNames(): toArray())
  let protected = augmentProtected(p, node: target(): parameterNames())
  if node: isVarargs() {
    ir = gololang.ir.invoke("varargs"): on(ir)
  }
  ir = gololang.ir.invoke("block")
...
local function quoteFunction = |node, p| {
  ...
  ir = gololang.ir.invoke("withParameters")
  ...
  if node: isMacro() {
    ir = gololang.ir.invoke("asMacro"): on(ir)
  }
  if node: isLocal() {
    ir = gololang.ir.invoke("local"): on(ir)
  }
  if node: isDecorated() {
    ir = gololang.ir.invoke("decoratedWith")
      : withArgs(quoteNodeArray(node: decorators(), p))
      : on(ir)
  }
  ir = gololang.ir.invoke("contextual")
    : withArgs(ConstantStatement.of(node: isContextualMacro()))
    : on(ir)
  ir = gololang.ir.invoke("special")
...
local function quoteDecorator = |node, p| {
  ...
  return gololang.ir.invoke("constant")
...
local function quoteNamedAugmentation = |node, p| -> gololang.ir.invoke("add")
    : withArgs(quoteNodeArray(node: functions(), p))
...
local function quoteDestruct = |node, p| {
  ...
  ir = gololang.ir.invoke("to")
    : withArgs(quoteNodeArray(node: references(), p))
    : on(ir)
  ir = gololang.ir.invoke("declaring")
    : withArgs(ConstantStatement.of(node: isDeclaring()))
    : on(ir)
  ir = gololang.ir.invoke("varargs")
    : withArgs(ConstantStatement.of(node: isVarargs()))
    : on(ir)
  if not node: isConstant() {
    ir = gololang.ir.invoke("variable"): on(ir)
  }
  return ir
}
...
----
Converts the given nodes to calls to builders functions.

See [`quoteNode`](#quoteNode_2).
----
macro quote = |nodes...| -> _quote_(set[], nodes)
