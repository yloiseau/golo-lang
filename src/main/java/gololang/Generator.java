 ...
 * @see LazyList
 */
public class Generator implements Iterator<Object>, AutoCloseable, Supplier<Optional<Object>>, HeadTail<Object> {
   ...
   * {@inheritDoc}
   *
   * <p>Returns the an option of the next element in the generator, and the empty option if the generator is finished.
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * <p>Starts the generator if needed and returns the current value.
   *
   ...
   * {@inheritDoc}
   *
   * <p>Fork the generator, starts the fork if needed, skips the first element, and returns it.
   * <p>A new fork is created for each call.
   * @see #skip()
   */
  @Override
  public Generator tail() {
    Generator forked = this.fork();
    if (forked.isEmpty()) {
      return forked;
    }
    if (!started) {
   ...
   * <p>
   * If an independent iterator is needed, use {@link #fork()}.
   */
